...
static int draw_frame(Bar *bar)
{
    ...
    if (bar->overview) {
        x = draw_text("OVERVIEW", x, y, foreground, foreground_mask, background,
                      &active_fg_color, &active_bg_color, bar->width,
        ...
        for (uint32_t i = 0; i < tags_l; i++) {
            ...
            x = draw_text(tags[i], x, y, foreground, foreground_mask,
                          background, fg_color, bg_color, bar->width,
                          bar->height, bar->textpadding, NULL, 0);
        }
    }
                           ...
                           uint32_t state)
{
    Seat *seat = (Seat *)data;
    seat->pointer_button =
        state == WL_POINTER_BUTTON_STATE_PRESSED ? button : 0;
}
...
static void pointer_frame(void *data, struct wl_pointer *pointer)
{
    ...
    if (!bar->overview) {
        do {
            ...
            x += TEXT_WIDTH(tags[i], bar->width - x, bar->textpadding) /
                 buffer_scale;
        ...
        if (i < tags_l) {
            if (ipc) {
                if (seat->pointer_button == BTN_LEFT)
                    ...
                    ipc_dispatch_tag(bar->xdg_output_name,
                                     bar->mtags ^ (1 << i));
            }
    ...
    x += TEXT_WIDTH(bar->layout, bar->width - x, bar->textpadding) /
         buffer_scale;
    if (seat->pointer_x < x) {
...
static void dwl_ipc_manager_tags(void *data, struct zdwl_ipc_manager_v2 *mgr,
                                 uint32_t count)
{
}
static void dwl_ipc_manager_layout(void *data, struct zdwl_ipc_manager_v2 *mgr,
                                   const char *name)
{
